...
class PgWorker(Worker):
    ...
    def write_assessments(self, asmts: [Assessment]):
        postgres_writer.write_records_to_table(self.db_connection, self.schema + '.dim_asmt', out_cfg.DIM_ASMT_FORMAT['columns'], asmts)

    def write_hierarchies(self, hierarchies: [InstitutionHierarchy]):
        postgres_writer.write_records_to_table(self.db_connection, self.schema + '.dim_inst_hier', out_cfg.DIM_INST_HIER_FORMAT['columns'], hierarchies)

    def write_students_dim(self, students: [Student]):
        postgres_writer.write_records_to_table(self.db_connection, self.schema + '.dim_student', out_cfg.DIM_STUDENT_FORMAT['columns'], students,
                                               entity_filter=('held_back', False))

    def write_students_reg(self, students: [Student], rs_guid, asmt_year):
        postgres_writer.write_records_to_table(self.db_connection, self.schema + '.student_reg', out_cfg.STUDENT_REG_FORMAT['columns'], students)

    def write_iab_outcome(self, results: [InterimAssessmentOutcome], guid):
        try:
            postgres_writer.write_records_to_table(self.db_connection, self.schema + '.fact_block_asmt_outcome', out_cfg.FBAO_FORMAT['columns'], results)
